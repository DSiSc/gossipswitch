...
func (st *StateTransition) TransitionDb() (ret []byte, usedGas uint64, failed bool, err error, contractAddr types.Address) {
	...
	*/
	...
	if contractCreation {
		...
		ret, addr, st.gas, vmerr = evm.Create(sender, st.data, math.MaxUint64, st.value)
	} else {
	...
	if vmerr != nil {
		...
		if vmerr == evmNg.ErrInsufficientBalance {
			return ret, 0, false, vmerr, addr
		}
	}

	return ret, st.gasUsed(), vmerr != nil, vmerr, addr
}
